    ...
    def enrich_paper(self, paper: Paper) -> Paper:
        ...
        try:
            ...
            if s2_paper:
                ...
                logger.debug(f"Enriched paper {paper.title[:50]}... with {paper.citation_count} citations")
            else:
                paper.citation_count = 0
                logger.debug(f"Could not find paper in SemanticScholar: {paper.title[:50]}...")
...
def enrich_papers(papers: List[Paper], 
                 semantic_scholar_api_key: Optional[str] = None,
                 github_token: Optional[str] = None) -> List[Paper]:
